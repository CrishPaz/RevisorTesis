...
async def send_with_attachments(
    *,
    ...
    """Envía un email con múltiples adjuntos opcionales.

    Args:
        attachments: Lista de tuplas (bytes, filename, mime_type).

    Raises EmailNotConfiguredError si faltan credenciales SMTP.
    Raises EmailDeliveryError en fallo de transporte.
    """
    settings = get_settings()
    if not (
        settings.smtp_host
    ...
    for attachment_bytes, attachment_filename, attachment_mime in (attachments or []):
        maintype, _, subtype = attachment_mime.partition("/")
        msg.add_attachment(
            attachment_bytes,
            maintype=maintype or "application",
